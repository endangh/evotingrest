...
public class PemilihServiceImpl implements PemilihService {
	...
	public Pemilih getPemilih(String nrp) {
		...
		try {
			...
			while (rs.next()) {
				p = new Pemilih(rs.getString("id_pemilih"), rs.getString("nama_pemilih"), rs.getString("no_hp_pemilih"), rs.getString("email_pemilih"), rs.getString("status_vote_pemilih"),rs.getBoolean("status_pemilih"));
			}
