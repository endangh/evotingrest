...
public class SuaraServiceImpl implements SuaraService{
	...
	public List<Suara> findAll() {
		...
		try {
			...
			while (rs.next()) {
				list.add(new Suara(rs.getInt("id_suara"), rs.getInt("id_kandidat"), rs.getString("nama_kanidat"), rs.getInt("jumlah_suara")));
			}
	...
	public boolean insertSuara(int idKandidat) {
		...
		String query = "update suara SET jumlah_suara = '"+(totalSuara+1)+"' where id_kandidat = '"+idKandidat+"'";
		try {
			toReturn = Koneksi.getInstance().getKoneksi().createStatement().execute(query);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int getSuara(int idKandidat) {
		// TODO Auto-generated method stub
		int total = 0;
		String query = "select jumlah_suara from suara where id_kandidat = '"+idKandidat+"'";
		try {
			ResultSet rs = Koneksi.getInstance().getKoneksi().createStatement().executeQuery(query);
			while (rs.next()) {
